...
def get_CAD_model_len(deepcad_json):
    """Return the number of operations in a DeepCAD model's sequence."""
    return len(deepcad_json["sequence"])


def filter_CAD_model_by_len(deepcad_json_list, max_len):
    """Return data_ids of models whose sequence length is <= max_len."""
    return [
        data_id
        for data_id, deepcad_json in deepcad_json_list.items()
        if get_CAD_model_len(deepcad_json) <= max_len
    ]


def cad_filter(max_len, deepcad_prefix_path=None):
    """
    ...
    if deepcad_prefix_path is None:
        deepcad_prefix_path = INPUT_DIR
    ...
    for short_path in tqdm(test_json_list, desc="Loading CAD models"):
        json_path = os.path.join(deepcad_prefix_path, f"{short_path}.json")
        deepcad_json_list[short_path] = load_deepcad(json_path)

    filter_data = filter_CAD_model_by_len(deepcad_json_list, max_len)
    print(filter_data)
